...
// ---------------------------------------------------------------------
enum class ProjectTemplate { Cpp, Python, Node };
enum class Editor { Vim, Emacs, Nano };
enum class TwoFactorMethod { App, Sms, Email };
...
void projectDelete(const std::string& name, bool force, const std::optional<std::string>& reason) {
    std::printf("[delete] project '%s' (force=%s, reason=%s)\n", name.c_str(),
                force ? "yes" : "no", reason ? reason->c_str() : "<none given>");
}
...
void build(const std::vector<int32_t>& targets, bool noAssets, bool debug, bool verbose,
    ...
    static const char* editorNames[] = {"vim", "emacs", "nano"};
    std::printf("[build] targets=[");
    for (size_t i = 0; i < targets.size(); ++i) std::printf("%s%d", i ? "," : "", targets[i]);
...
void deploy(bool dryRun, const std::optional<std::string>& env, const std::vector<std::string>& hosts) {
    std::printf("[deploy] dryRun=%d env=%s hosts=[", dryRun, env ? env->c_str() : "<none>");
    for (size_t i = 0; i < hosts.size(); ++i) std::printf("%s%s", i ? "," : "", hosts[i].c_str());
    std::printf("]\n");
}
