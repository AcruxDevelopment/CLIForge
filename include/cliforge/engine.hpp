...
class Engine {
public:
    explicit Engine(std::string programName = "") : programName_(std::move(programName)) {}
    ...
    int run(int argc, char** argv) {
        if (programName_.empty() && argc > 0) programName_ = basename(argv[0]);
        ...
        } else if (auto it = std::find_if(tokens.begin(), tokens.end(),
                                           [](const std::string& t) { return t == "--help" || t == "-h"; });
                   it != tokens.end()) {
        ...
        for (auto& c : candidates) {
            ...
            try {
                ...
                for (auto& v : values) all.push_back(std::move(v));
        ...
        if (firstErrorCmd) std::cerr << "Usage: " << programName_ << " " << firstErrorCmd->usageLine() << "\n";
        return 1;
    }
...
private:
    ...
    int showHelp(const std::vector<std::string>& queryTokens) const {
        ...
        for (auto& cmd : commands_) {
            if (cmd->couldMatchPrefix(queryTokens)) matches.push_back(cmd.get());
        }
        if (matches.empty()) {
        ...
        std::cout << "'" << join(queryTokens) << "' matches " << matches.size()
                   << " commands:\n\n";
    ...
    void suggestCommands(const std::vector<std::string>& tokens, std::ostream& os) const {
        ...
        for (auto& cmd : commands_) {
            ...
            double ratio = s.keywordChars ? static_cast<double>(s.distance) / static_cast<double>(s.keywordChars)
                                           : 1.0;
        ...
        for (auto& [ratio, cmd] : scored) {
            ...
            if (ratio <= 0.5) close.push_back(cmd);
        }
        if (close.empty()) return;
        os << "\nDid you mean:\n";
        for (auto* cmd : close) os << "  " << programName_ << " " << cmd->usageLine() << "\n";
    }
