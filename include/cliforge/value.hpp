...
public:
    explicit ParseError(const std::string& msg) : CliError(msg, ErrorKind::TypeMismatch) {}
};
...
struct RegistrationError : CliError {
    explicit RegistrationError(const std::string& msg) : CliError(msg, ErrorKind::Generic) {}
};
...
using Scalar = std::variant<int8_t, uint8_t, int16_t, uint16_t, int32_t,
                             uint32_t, int64_t, uint64_t, float, double,
                             std::string, char, bool, EnumTag>;

enum class Kind {
    I8, U8, I16, U16, I32, U32, I64, U64, F32, F64, Str, Char, Bool, Enum
};
...
struct TypeInfo {
    ...
    bool valid() const { return parsePlain != nullptr || choiceTable != nullptr; }
...
Float parseFloating(std::string_view token, std::string_view label, std::string_view typeName) {
    Float out{};
    auto [ptr, ec] = std::from_chars(token.data(), token.data() + token.size(), out);
    if (ec != std::errc{} || ptr != token.data() + token.size()) {
        throw ParseError("type mismatch for '" + std::string(label) + "': got '" +
...
template <typename T>
struct TypeOf;  // intentionally undefined for unsupported T
...
struct TypeOf<bool> {
    static TypeInfo get() {
        ...
        t.parsePlain = [](std::string_view tok, std::string_view lbl) -> Scalar {
            ...
            throw ParseError("type mismatch for '" + std::string(lbl) + "': got '" +
                              std::string(tok) + "', expected bool (true/false, 1/0, yes/no, on/off)");
        };
        return t;
    }
};
...
public:
    ...
    bool isVector() const { return std::holds_alternative<std::vector<Scalar>>(data_); }
